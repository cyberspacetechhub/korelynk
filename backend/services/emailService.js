...
class EmailService {
  ...
  async sendAutoReply(contactData) {
    ...
    const autoReply = {
      from: process.env.EMAIL_USER,
      to: email,
      subject: 'Thank you for contacting InnTechLab',
      html: `
        ...
        <p>${message}</p>
        <br>
        <p>Best regards,<br>InnTechLab Team</p>
      `
    }
  ...
  async sendNewsletterWelcome(email) {
    const welcomeEmail = {
      from: process.env.EMAIL_USER,
      to: email,
      subject: 'Welcome to InnTechLab Newsletter',
      html: `
        <h2>Welcome to our newsletter!</h2>
        <p>Thank you for subscribing to InnTechLab newsletter.</p>
  ...
  async sendPasswordResetCode(email, code, userType = 'admin') {
    const resetEmail = {
      from: process.env.EMAIL_USER,
      to: email,
      subject: 'Password Reset Code - InnTechLab',
      html: `
        <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif; padding: 20px;">
          <h2 style="color: #4f46e5; text-align: center;">Password Reset Request</h2>
          <p>You have requested to reset your password for InnTechLab ${userType} portal.</p>
          ...
          <p style="font-size: 12px; color: #6b7280; text-align: center;">
            This is an automated message from InnTechLab<br>
  ...
  async sendVerificationCode(email, code, userType) {
    ...
    const verificationEmail = {
      from: process.env.EMAIL_USER,
      to: email,
      subject: 'Email Verification - InnTechLab',
      html: `
        <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif; padding: 20px;">
          <h2 style="color: #4f46e5; text-align: center;">Verify Your Email</h2>
          <p>Welcome to InnTechLab! Please verify your email to complete your ${userType} account setup.</p>
          <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; text-align: center; margin: 20px 0;">
  ...
  async sendWelcomeEmail(email, fullName, userType) {
    const welcomeEmail = {
      from: process.env.EMAIL_USER,
      to: email,
      subject: `Welcome to InnTechLab - ${userType} Account Created`,
      html: `
        <div style="max-width: 600px; margin: 0 auto; font-family: Arial, sans-serif; padding: 20px;">
          <h2 style="color: #4f46e5; text-align: center;">Welcome to InnTechLab!</h2>
          <p>Dear ${fullName},</p>
